...
@app.route('/user/<find_key>/<select_salary>/<select_area>/<related_key>')
def show_user_profile(find_key, select_salary, select_area, related_key):
    print(select_area) # input 11,22
    # print(type(select_area))
    print(select_area.split(','))
    select_area = select_area.split(',')
    print(type(select_area))
    thread = threading.Thread(target=email_find.email, args=(find_key, select_salary, select_area, related_key, ))   # 增加定義線程
    thread.start()  # 讓線程開始工作 
    # show the user profile for that user
    return '爬取資料 %s %s %s %s ' % (find_key, select_salary, select_area, related_key)
...
使用方法：
例子 ：你的heroku網址/user/金融/30000/台北市,新北市,桃園市/分析
...
例如：
    ...
    save_excel = concat_104_1111.loc[(mask1 & mask2 & mask5) | (mask1 & mask3 & mask5) | (mask1 & mask4 & mask5)] | (mask1 & mask6 & mask5)]
